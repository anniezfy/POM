...

{
    ...
    else
    {
        ...
        if (true)
        {
            ...
            DEBUG(3, std::cout << "Saved variable " << this->name << " of type " << str_from_polyfp_type_primitive(this->dtype));
        }
    }
}
...
polyfp::var::var(std::string name, polyfp::primitive_t type)
{
    ...
    else
    {
        ...
        {
            var::declared_vars.insert(std::make_pair(name, *this));
            DEBUG(10, std::cout << "Saved variable " << this->name << " of type " << str_from_polyfp_type_primitive(this->dtype));
        }
    }
}
...
//std::unordered_map<std::string, var> polyfp::var::declared_vars;
...
expr polyfp::expr::operator>>(polyfp::expr other) const {
    return polyfp::expr{o_right_shift, *this, other};
}

expr polyfp::expr::operator<<(polyfp::expr other) const {
    return polyfp::expr{o_left_shift, *this, other};
}
// todo
